...
var overviewTab = {
  ...
  renderStats: function (data) {
    ...
    html += '<div class="px-1">';
    html +=
      '<h3 class="text-lg font-black text-slate-900 dark:text-white truncate leading-tight mb-1">' +
    ...
    var hasTwitterData = data.meta && data.meta.twitter && (data.meta.twitter.title || data.meta.twitter.description || data.meta.twitter.image);
    if (hasOgData || hasTwitterData) {
       html += '<div class="mt-6">';
       html +=
         '<h4 class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Link Preview</h4>';
       
       // Preview Card
       html += '<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">';
       
        // Image (if available)
        var previewImg = data.meta.image || "";
        if (previewImg) {
          html +=
       ...
       // Title
       var cardTitle = data.meta.og.title || data.meta.twitter.title || data.meta.title || "";
       if (cardTitle) {
         html +=
           '<h5 class="text-sm font-bold text-slate-900 dark:text-white leading-snug mb-2 line-clamp-2">' +
           self.escapeHtml(cardTitle) +
           "</h5>";
       }
       
       // Description
       var cardDesc = data.meta.og.description || data.meta.twitter.description || data.meta.description || "";
       if (cardDesc) {
         html +=
           '<p class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-3 mb-3">' +
           self.escapeHtml(cardDesc) +
           "</p>";
       }
       
       // URL / Domain
       var displayUrl = data.meta.og.url || data.meta.url || data.host || "";
       if (displayUrl) {
         html +=
           '<div class="flex items-center gap-2 text-[10px] font-mono text-slate-500 dark:text-slate-400 truncate">';
          // Favicon
    ...
    html += '<div class="mt-8">';
    html +=
      '<h4 class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4">Typography</h4>';
    html += '<div class="space-y-3">';

    var fonts = data.typography || [];
    var headings = "No headings found";
    var body = "Not detected";
    ...
    html +=
      '<div class="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm hover:border-brand-300 transition-all flex items-center justify-between group">';
    html += "<div>";
    html +=
      '<div class="text-[9px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest mb-1">HEADINGS</div>';
    html +=
      '<div class="text-[15px] font-black text-slate-900 dark:text-white">' +
      headings +
      "</div></div>";
    html +=
      '<div class="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-brand-600 dark:text-brand-400 font-black">H1</div>';
    html += "</div>";
    ...
    html += "<div>";
    html +=
      '<div class="text-[9px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest mb-1">BODY TEXT</div>';
    html +=
      '<div class="text-[15px] font-black text-slate-900 dark:text-white">' +
      body +
      "</div></div>";
    html +=
      '<div class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 font-bold">Aa</div>';
    html += "</div>";
    html += "</div></div>";
    ...
    html += '<div class="flex items-center justify-between mb-4">';
    html +=
      '<h4 class="text-[11px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest">Color Palette</h4>';
    html +=
    ...
    for (var m = 0; m < Math.min(colors.length, 12); m++) {
      html +=
        '<div class="aspect-square rounded-lg border border-slate-200/50 dark:border-slate-700 shadow-sm flex-shrink-0 cursor-pointer hover:scale-110 transition-transform" style="background-color:' +
        colors[m].color +
        '" title="' +
        colors[m].color +
        '"></div>';
    ...
    if (colors.length === 0)
      html +=
        '<div class="col-span-full text-[10px] text-slate-300 font-bold italic uppercase tracking-widest opacity-40">Scanning for colors...</div>';
    html += "</div></div>";
    ...
    html +=
      '<h4 class="text-[11px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest">Contrast Scanner</h4>';
    html +=
    ...
    if (issueCount > 0) {
      ...
      html +=
        '<div class="p-5 bg-white border border-slate-100 dark:border-slate-800 rounded-[28px] shadow-sm group hover:border-brand-100 transition-all">';
      html += '<div class="flex items-center justify-between mb-4">';
      ...
      html +=
        '<div class="text-[9px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest mb-2">TOP CONTRAST ISSUES</div>';

      for (var x = 0; x < Math.min(issueCount, 3); x++) {
        var issue = data.contrastIssues[x];
        html +=
          '<div class="flex items-center justify-between text-[10px] bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-800">';
        html +=
          '<span class="font-bold text-slate-800 dark:text-slate-100 truncate w-32">' +
          issue.selector +
          "</span>";
        html +=
      ...
      html +=
        '<button class="w-full mt-2 py-2 text-[10px] font-black text-brand-500 uppercase tracking-widest cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl transition-all" id="toggle-contrast-details">See all issues</button>';
      html += "</div>";
      ...
      for (var y = 0; y < data.contrastIssues.length; y++) {
        ...
        html +=
          '<div class="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 flex flex-col gap-1">';
        html += '<div class="flex items-center justify-between">';
        html +=
          '<span class="text-[10px] font-bold text-slate-800 dark:text-slate-100 truncate w-32">' +
          issue.selector +
          "</span>";
    ...
    html += "</div>";
     ...
     html += '<div class="mt-8">';
     html +=
       '<h4 class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4">CSS Metrics</h4>';
    ...
    for (var n = 0; n < stats.length; n++) {
      html +=
        '<div class="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm hover:border-brand-300 transition-all group">';
      html +=
        ...
        '"></path></svg>';
      html +=
        '<span class="text-[9px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest">' +
    ...
    if (contrastToggle) {
      contrastToggle.onclick = function () {
        ...
        if (details) {
          var isHidden = details.classList.toggle("hidden");
